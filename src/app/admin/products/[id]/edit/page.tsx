// app/admin/products/[id]/edit/page.tsx - UPDATED WITH CLOUDINARY
'use client';
...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Initialize uploaded images with existing product images
        const existingImages: CloudinaryImage[] = (product.images || []).map(url => ({
          url,
          publicId: undefined // We don't have publicId for existing images
        }));
        setUploadedImages(existingImages);

      } else {
        const errorData = await response.json();
        toast.error(errorData.error || 'Failed to fetch product');
      }
    } catch (error) {
  ...
  const removeImage = async (index: number) => {
    ...
    // Only delete from Cloudinary if it's a newly uploaded image (has publicId)
    // Don't delete original product images to avoid breaking other references
    if (imageToRemove?.publicId && !originalImages.includes(imageToRemove.url)) {
      try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        toast.error(data.error || 'Failed to update product', { id: saveToast });
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border p-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            {formData.images.length > 0 && (
              <div className="mt-4">
                <p className="text-sm text-gray-600 mb-3">Product Images ({formData.images.length}):</p>
                  ...
                  {formData.images.map((image, index) => (
                    <div key={index} className="relative group">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs p-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {originalImages.includes(image) ? 'Original' : 'Cloudinary'}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
          ...
          <button
            type="submit"
            disabled={saving || formData.images.length === 0 || !formData.category}
